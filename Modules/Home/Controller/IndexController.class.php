...
class IndexController extends Controller {
    ...
    public function index(){
		...
		S('site_index',$site_index,$this->cacheTime);
		echo $site_index;
    }
	//内容列表页及栏目页
	public function lists()
	{
		...
		$html_info=S('site_lists_'.$catid);
		if($html_info)die($html_info);
		$this->_init();
		$catinfo=$this->d_category->where(array('id'=>$catid))->field('name,parentid,modelids,template1')->find();
		...
		$this->assign($seo);	
		$html_info=$this->fetch('Lists/'.$catinfo['template1']);
		S('site_lists_'.$catid,$html_info,$this->cacheTime);
		echo $html_info;
	}
	//内容详细页
	public function detail()
	{
		...
		$html_info=S('site_lists_'.$catid.'_'.$id);
		if($html_info)die($html_info);
		$this->_init();		
		//栏目信息
		$catinfo=$this->d_category->where(array('id'=>$catid))->field('name,parentid,modelids,template2')->find();
		$this->assign('name',$catinfo['name']);
		$this->assign('parentid',$catinfo['parentid']);
		$this->assign('catid',$catid);
		$this->assign('pname',$this->d_category->getName($catinfo['parentid']));
		...
		$html_info=$this->fetch('Detail/'.$catinfo['template2']);
		S('site_lists_'.$catid.'_'.$id,$html_info,$this->cacheTime);
		echo $html_info;
	}
	//初始化网站
