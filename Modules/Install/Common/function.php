...
function sp_create_admin_account($db,$table_prefix){
    ...
    $password=password(I("post.manager_pwd"));
    $email=I("post.manager_email");
    $sql =<<<hello
    INSERT INTO `{$table_prefix}admin` 
    (userid,username,password,encrypt,email,status) VALUES 
    ('1', '{$username}', '{$password["password"]}', '{$password["encrypt"]}' ,'{$email}', '1');;
hello;
    $db->execute($sql);
...
function sp_create_config($config, $authcode){
    if(is_array($config)){
        ...
        $conf = str_replace('#COOKIE_PREFIX#', create_randomstr(6) . "_", $conf);

        //写入应用配置文件
        if(file_put_contents( 'Conf/db.php', $conf)){
