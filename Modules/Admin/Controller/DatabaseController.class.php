...
class DatabaseController extends \Admin\Classes\AdminController {
    ...
    public function index() {
        ...
        include($this->admin_tpl('Database/index'));
    }
    ...
    public function backup() {
        ...
        $time = time();//开始时间
        $path = $this->getDbPath() . "/thinkerphptables_" . date("Ymd") . "_" . create_randomstr(5);
    ...
    public function restore() {
       ...
       include($this->admin_tpl('Database/restore'));
    }
            ...
            while (!feof($file)) {
                ...
                if ($tem[$end] == ";") {
                    ...
                    if ($execute > 500) {
                        ...
                        //echo json_encode(array("status" => 1, "info" => '如果导入SQL文件卷较大(多)导入时间可能需要几分钟甚至更久，请耐心等待导入完成，导入期间请勿刷新本页，当前导入进度：<font color="red">已经导入' . $imported . '条Sql</font>', "url" => U('Database/restoreData', array(create_randomstr(5) => create_randomstr(5)))));
                        $this->success('如果SQL文件卷较大(多),则可能需要几分钟甚至更久,<br/>请耐心等待完成，<font color="red">请勿刷新本页</font>，<br/>当前导入进度：<font color="red">已经导入' . $imported . '条Sql</font>', U('Database/restoreData', array(create_randomstr(5) => create_randomstr(5))));                       
                        exit;
                    }
                } else {
                    $sql.=$tem;
