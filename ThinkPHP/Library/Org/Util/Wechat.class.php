 ...
 * 服务器端必须要有 CURL 支持
 * 2015年4月修正版本
 * @author 、thinker <thinkerphp@thinkerphp.com>
...
class Wechat {
	...
	const GROUP_DELETE_URL        = 'https://api.weixin.qq.com/cgi-bin/groups/delete';
	/*素材管理*/
	const MATERiAL_LISTS_URL      = 'https://api.weixin.qq.com/cgi-bin/material/batchget_material';
	const GET_INT_MEDIA_URL       = 'https://api.weixin.qq.com/cgi-bin/media/get';
	/* 发送客服消息URL */
	...
	public function valid() {
		if($_GET["echostr"])$echoStr = safe_replace($_GET["echostr"]);	
		if (isset($echoStr)) {
	...
	/**
	 * 检查用户签名信息	 * @author 、thinker <thinkerphp@thinkerphp.com>
	 */
	...
	public function group_delete($id = '') {
		if (empty($id)) {
			$this->error = '请输入一个分组ID';
			return false;
		}
		$params = '{"group":{"id":'.$id.'}}';
		$url    = self::GROUP_DELETE_URL . '?access_token=' . $this->access_token;
		$jsonStr = $this->http($url, $params, 'POST');
		$jsonArr = $this->parseJson($jsonStr);
		if ($jsonArr) {
			return $jsonArr['group'];
	 ...
	 * @author 、thinker <thinkerphp@thinkerphp.com>
	 */
	public function material_lists($type='image',$offset=0,$count=20) {
		$params = array(
				'type'=>$type,
				'offset'=>$offset,
				'count'=>$count
	...
	//获取临时的文件存放
	public function get_int_media_url($media_id)
	{
		return self::GET_INT_MEDIA_URL . '?access_token=' . $this->access_token .'&media_id='.$media_id;
	}
	/**
	...
	//获取地址地址签名
	public function getAddressSignPackage()
	{	
		...
		$Parameters['accesstoken'] = $token['access_token'];
		$Parameters['addrsign'] = $this->genSha1Sign($Parameters);
		return $Parameters;		
	}	
