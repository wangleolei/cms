...
class Aliyunocs extends Cache
{
    ...
    public function __construct($options = array())
    {
        ...
        $options = array_merge(array(
            'servers' => C('OCS.HOST') ? C('OCS.HOST') : null,
            'port' => C('OCS.PORT') ? C('OCS.PORT') : 11211,
            'lib_options' => C('MEMCACHED_LIB') ? C('MEMCACHED_LIB') : null,
            'username'        =>  C('OCS.USERNAME') ? C('OCS.USERNAME') : 'username',
            'password'        =>  C('OCS.PASSWORD') ? C('OCS.PASSWORD') : 'password',
        ), $options);
